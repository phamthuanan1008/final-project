...
const AddCategoryProduct = () => {
  ...
  useEffect(() => {
    ...
    fetch(linkApi + `/api/category-product/get/all/pagination?size=${sizePage}`,{headers:{"Authorization": `Bearer ${jwt}`}})
      .then((res) => res.json())
      .then((data) => {
    ...
    fetch(linkApi + `/api/category-product/get/all/pagination?page=${currentPage}&size=${sizePage}&sort=categoryId,desc`,{
      headers:{
      ...
      .then((data) => {
        if (data.code === 200) {
          setCategoryProduct(data.data.content);
        } else {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!error) {
      const apiUrl = editingCategory
        ? `${linkApi}/api/category-product/update/${editingCategory.categoryId}?userId=${user?.userId}`
        : `${linkApi}/api/category-product/add?userId=6`;
  ...
  const handleDelete = (categoryId: number) => {
    ...
    if (confirmation) {
      fetch(linkApi + `/api/category-product/delete/${categoryId}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${jwt}`,
        },
  ...
  const handleEdit = (categoryId: number) => {
    const categoryToEdit = categoryProduct.find((category) => category.categoryId === categoryId);
    if (categoryToEdit) {
