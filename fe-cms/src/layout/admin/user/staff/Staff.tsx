...
const UserList = () => {
  ...
  useEffect(() => {
    fetch(linkApi + `/api/user/get/by/authorize-name?authorizeName=STAFF&size=${sizePage}`, {
      headers: {
    ...
    fetch(linkApi + `/api/user/get/by/authorize-name?authorizeName=STAFF&page=${currentPage}&size=${sizePage}&sort=userId,desc`, {
      headers: {
        "Authorization": `Bearer ${jwt}`
      }
    }).then((res) => res.json())
